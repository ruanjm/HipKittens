    ...
    const GL& src, const COORD& idx, ST& dst, uint32_t* swizzled_offsets)
{
    ...
    constexpr int num_subtiles_per_tile = 6;
    constexpr int num_normal_subtiles_per_tile = 4;
    constexpr int num_special_subtiles_per_tile = 2;
    constexpr int num_normal_subtile_rows = 16;
    constexpr int bytes_per_normal_subtile_row = 16;
    constexpr int num_special_subtile_rows = 32;
    constexpr int bytes_per_special_subtile_row = 8;
    constexpr int bytes_per_subtile = bytes_per_base_tile / num_subtiles_per_tile;
    ...
    for (int i = 0; i < memcpy_per_tile; i++) {
        ...
        const int subtile_byte_offset = base_tile_byte_offset % bytes_per_subtile;
        const int subtile_id = base_tile_byte_offset / bytes_per_subtile;

        const int is_a_normal_subtile = subtile_id < num_normal_subtiles_per_tile;
        const int normal_subtile_id = subtile_id % num_normal_subtiles_per_tile;
        const int special_subtile_id = subtile_id % num_special_subtiles_per_tile;

        const int base_tile_row_offset = is_a_normal_subtile ? 
            (normal_subtile_id % 2) * num_normal_subtile_rows + (subtile_byte_offset / bytes_per_normal_subtile_row) :
            (subtile_byte_offset / bytes_per_special_subtile_row);

        const int base_tile_col_byte_offset = is_a_normal_subtile ?
            (normal_subtile_id / 2) *  (bytes_per_normal_subtile_row + bytes_per_special_subtile_row) + (subtile_byte_offset % bytes_per_normal_subtile_row) :
            bytes_per_normal_subtile_row + (special_subtile_id * (bytes_per_normal_subtile_row + bytes_per_special_subtile_row)) + (subtile_byte_offset % bytes_per_special_subtile_row);
 ...
 */
 ...
 __device__ inline static void load_lds_reg_row_fp6_shuffled(RT &dst, const ST &src) {
     ...
     constexpr int num_normal_subtile_rows = 16;
     constexpr int bytes_per_normal_subtile_row = 16;
     constexpr int bytes_per_normal_subtile = num_normal_subtile_rows * bytes_per_normal_subtile_row;
     constexpr int num_special_subtile_rows = 32;
     constexpr int bytes_per_special_subtile_row = 8;
     constexpr int bytes_per_special_subtile = num_special_subtile_rows * bytes_per_special_subtile_row;

     const int normal_subtile_id = laneid / num_normal_subtile_rows;
     const int special_subtile_id = laneid / num_special_subtile_rows;

     const int normal_row_offset = laneid % num_normal_subtile_rows;
     const int special_row_offset = laneid % num_special_subtile_rows;

     const int normal_byte_offset = (normal_subtile_id * bytes_per_normal_subtile) + (normal_row_offset * bytes_per_normal_subtile_row);
     const uint32_t addr = reinterpret_cast<uintptr_t>(lds_bytes + normal_byte_offset);

     const int special_byte_offset = (num_normal_subtiles_per_tile * bytes_per_normal_subtile) + (special_subtile_id * bytes_per_special_subtile) + (special_row_offset * bytes_per_special_subtile_row);
     const uint32_t addr_b64 = reinterpret_cast<uintptr_t>(lds_bytes + special_byte_offset);
