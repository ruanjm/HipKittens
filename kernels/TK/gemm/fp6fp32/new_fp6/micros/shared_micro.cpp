...
void micro_tk(const micro_globals g) {
    ...
    // constexpr size_t FP6_TILE_BYTES = (BLOCK_SIZE * K_STEP * 6 + 7) / 8;
    // void* tile_fp6_base = al.allocate_fp6_
    // auto get_tile = [tile_fp6_base, FP6_TILE_BYTES](int idx) -> st_f6<BLOCK_SIZE, K_STEP>& {
    //     return *reinterpret_cast<st_f6<BLOCK_SIZE, K_STEP>*>(
    //         (char*)tile_fp6_base + idx * FP6_TILE_BYTES
    //     );
    // };

    // int condition = (threadIdx.x == 0 && blockIdx.x == 0 && blockIdx.y == 0);
    // if (condition) {
    //     printf("FP6_TILE_BYTES: %lu bytes (%lu KB)\n", FP6_TILE_BYTES, FP6_TILE_BYTES / 1024);
    // }
