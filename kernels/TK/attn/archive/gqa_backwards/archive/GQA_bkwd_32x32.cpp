...
template<int D, typename T=bf16, typename L=row_l, typename M=mfma_32x32x16> using qo_tile = rt<T, DOT_SLICE_QO, D, L, M>;
template<int D, typename T=bf16, typename L=row_l, typename M=mfma_32x32x16> using kv_tile = rt<T, WARP_SIZE_KV, D, L, M>;
template<int D, typename T=bf16, typename L=row_l, typename M=mfma_32x32x16> using qo_tile_T_dq = rt<T, 32, 32, L, M>;
template<int D, typename T=bf16, typename L=row_l, typename M=mfma_32x32x16> using qo_tile_dq = rt<T, 32, 32, L, M>;
template<int D, typename T=bf16, typename L=row_l, typename M=mfma_32x32x16> using kv_tile_T = rt<T, D, WARP_SIZE_KV, L, M>;
template<int D, typename T=float, typename L=accum_col_l, typename M=mfma_32x32x16> using attn_tile = rt<T, DOT_SLICE_QO, WARP_SIZE_KV, L, M>;
template<int D, typename T=bf16, typename L=col_l, typename M=mfma_32x32x16> using attn_tile_T = rt<T, WARP_SIZE_KV, DOT_SLICE_QO, L, M>;

template<int D, typename T=bf16, typename L=col_l, typename M=mfma_32x32x16> using attn_tile_T_dq = rt<T, 256, 32, L, M>;
template<int D, typename T=bf16, typename L=row_l, typename M=mfma_32x32x16> using kv_tile_dq = rt<T, 256, 32, L, M>;
...
__global__ void attend_bwd_combined_ker(const attn_bwd_combined_globals<D> g) {
    ...
    st_bf<BLOCK_SIZE_KV, D, ducks::st_layout::accumulator, ducks::st_matrix::mfma_32x32x16> (&K_j_smem) = al.allocate<st_bf<BLOCK_SIZE_KV, D, ducks::st_layout::accumulator, ducks::st_matrix::mfma_32x32x16>>();
    st_bf<DOT_SLICE_QO, D, ducks::st_layout::classical, ducks::st_matrix::mfma_32x32x16> (&Q_i_smem)[2][2] = al.allocate<st_bf<DOT_SLICE_QO, D, ducks::st_layout::classical, ducks::st_matrix::mfma_32x32x16>, 2, 2>();
    st_bf<DOT_SLICE_QO, D, ducks::st_layout::classical, ducks::st_matrix::mfma_32x32x16> (&dO_i_smem)[2][2] = al.allocate<st_bf<DOT_SLICE_QO, D, ducks::st_layout::classical, ducks::st_matrix::mfma_32x32x16>, 2, 2>();
    ...
    // Register tiles
    kv_tile<D, bf16, row_l, mfma_32x32x16> K_j, V_j;
    ...
    qo_tile<D, bf16, row_l, mfma_32x32x16> Q_i, dO_i;
    qo_tile<D, bf16, col_l, mfma_32x32x16> Q_i_col, dO_i_col;
    qo_tile_dq<D, float, accum_row_l> dQ_i;
    attn_tile<D, float, accum_col_l, mfma_32x32x16>::col_vec L_i, delta_i;
    ...
    for (int i = 0; i < num_steps - 1; ++i, tic ^= 1, toc ^= 1) {
        ...
        // dot slice 0
        {
            ...
            load(dO_i_col, dO_i_smem[tic][0]);
            load(Q_i_col, Q_i_smem[tic][0]);
            asm volatile("s_waitcnt lgkmcnt(0)");
            __builtin_amdgcn_s_barrier();
            __builtin_amdgcn_sched_barrier(0);
            ...
            atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, q_head_idx, q_seq_idx * 2, 0}, warpid);
            __builtin_amdgcn_s_setprio(0);
            __builtin_amdgcn_s_barrier();
            __builtin_amdgcn_sched_barrier(0);
        }
        // dot slice 1
        {
            ...
            load(dO_i_col, dO_i_smem[tic][1]);
            load(Q_i_col, Q_i_smem[tic][1]);
            asm volatile("s_waitcnt lgkmcnt(0)");
            __builtin_amdgcn_s_barrier();
            __builtin_amdgcn_sched_barrier(0);
            ...
            mma_AtB(dQ_i_T, K_j_col, dP_ij_bf16_col_T,  dQ_i_T);
            swap_layout_and_transpose(dQ_i, dQ_i_T);
            atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, q_head_idx, q_seq_idx * 2 + 1, 0}, warpid);
            __builtin_amdgcn_s_setprio(0);
            __builtin_amdgcn_s_barrier();
            __builtin_amdgcn_sched_barrier(0);
        }
    ...
    // dot slice 0
    {
        ...
        mma_AtB(dQ_i_T, K_j_col, dP_ij_bf16_col_T,  dQ_i_T);
        swap_layout_and_transpose(dQ_i, dQ_i_T);
        atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, first_q_head + GROUP_SIZE - 1, (num_steps_per_head - 1) * 2, 0}, warpid);
        __builtin_amdgcn_s_setprio(0);
        __builtin_amdgcn_s_barrier();
        __builtin_amdgcn_sched_barrier(0);
    }
    // dot slice 1
    {
        ...
        mma_AtB(dQ_i_T, K_j_col, dP_ij_bf16_col_T,  dQ_i_T);
        swap_layout_and_transpose(dQ_i, dQ_i_T);
        atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, first_q_head + GROUP_SIZE - 1, (num_steps_per_head - 1) * 2 + 1, 0}, warpid);
        __builtin_amdgcn_s_setprio(0);
        __builtin_amdgcn_s_barrier();
        __builtin_amdgcn_sched_barrier(0);
    }
