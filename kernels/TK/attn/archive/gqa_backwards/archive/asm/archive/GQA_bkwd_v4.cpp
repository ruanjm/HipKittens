...
__global__ __attribute__((amdgpu_num_vgpr(32))) void attend_bwd_combined_ker(const attn_bwd_combined_globals<D> g) {
  ...
  const int step_offset = (num_steps / 1) * (batch_idx / (ATTN_B / 1));
  ...
  const int q_head_idx = step_offset / num_steps_per_head + first_q_head;
  const int q_seq_idx = step_offset % num_steps_per_head;
  load(L_smem[tic], g.L_vec, {batch_idx, q_head_idx, 0, q_seq_idx});
  load(delta_smem[tic], g.delta_vec, {batch_idx, q_head_idx, 0, q_seq_idx});
  G::load<1, false>(Q_i_smem[tic][0], g.Q, {batch_idx, q_seq_idx * 2, q_head_idx, 0});
  G::load<1, false>(dO_i_smem[tic][0], g.dOg, {batch_idx, q_seq_idx * 2, q_head_idx, 0});
  G::load<1, false>(Q_i_smem[tic][1], g.Q, {batch_idx, q_seq_idx * 2 + 1, q_head_idx, 0});
  G::load<1, false>(dO_i_smem[tic][1], g.dOg, {batch_idx, q_seq_idx * 2 + 1, q_head_idx, 0});
  ...
  // Prologue
  {
    ...
    const int next_q_head_idx = (step_offset + 1) / num_steps_per_head + first_q_head;
    const int next_q_seq_idx = (step_offset + 1) % num_steps_per_head;
  ...
  for (int i = 1; i < num_steps - 1; ++i, tic ^= 1, toc ^= 1) {
    const int last_q_head_idx = ((step_offset + i - 1) % num_steps) / num_steps_per_head + first_q_head;
    const int last_q_seq_idx = ((step_offset + i - 1) % num_steps) % num_steps_per_head;

    const int q_head_idx = ((step_offset + i) % num_steps) / num_steps_per_head + first_q_head;
    const int q_seq_idx = ((step_offset + i) % num_steps) % num_steps_per_head;

    const int next_q_head_idx = ((step_offset + i + 1) % num_steps) / num_steps_per_head + first_q_head;
    const int next_q_seq_idx = ((step_offset + i + 1) % num_steps) % num_steps_per_head;
  ...
  // Epilogue
  {
    const int last_q_head_idx = ((step_offset + num_steps - 2) % num_steps) / num_steps_per_head + first_q_head;
    const int last_q_seq_idx = ((step_offset + num_steps - 2) % num_steps) % num_steps_per_head;

    const int q_head_idx = ((step_offset + num_steps - 1) % num_steps) / num_steps_per_head + first_q_head;
    const int q_seq_idx = ((step_offset + num_steps - 1) % num_steps) % num_steps_per_head;
