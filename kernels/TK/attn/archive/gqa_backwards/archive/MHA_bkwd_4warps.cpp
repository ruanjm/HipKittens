...
__global__ void attend_bwd_combined_ker(const attn_bwd_combined_globals<D> g) {
    ...
    // dot slice 0
    {
        load(L_smem[toc], g.L_vec, {batch_idx, q_head_idx, 0, 1});
        G::load<1, false>(Q_i_smem[toc][0], g.Q, {batch_idx, 2, q_head_idx, 0});
    ...
    // dot slice 1
    {
        load(delta_smem[toc], g.delta_vec, {batch_idx, q_head_idx, 0, 1});
        G::load<1, false>(dO_i_smem[toc][0], g.dOg, {batch_idx, 2, q_head_idx, 0});
        load(K_j, subtile_inplace<WARP_SIZE_KV, D>(K_j_smem, {warpid, 0}));
        ...
        atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, q_head_idx, 0, 0}, warpid);
        auto attn_i_smem_subtile = subtile_inplace<WARP_SIZE_KV, DOT_SLICE_QO>(attn_i_smem, {warpid, 0});
    ...
    // dot slice 2
    {
        G::load<1, false>(Q_i_smem[toc][1], g.Q, {batch_idx, 3, q_head_idx, 0});
        load(K_j, subtile_inplace<WARP_SIZE_KV, D>(K_j_smem, {warpid, 0}));
        ...
        atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, q_head_idx, 1, 0}, warpid);
        auto attn_i_smem_subtile = subtile_inplace<WARP_SIZE_KV, DOT_SLICE_QO>(attn_i_smem, {warpid, 0});
        store(attn_i_smem_subtile, dP_ij_bf16_accum_row);
    ...
    // dot slice 3
    {
        G::load<1, false>(dO_i_smem[toc][1], g.dOg, {batch_idx, 3, q_head_idx, 0});
        load(K_j, subtile_inplace<WARP_SIZE_KV, D>(K_j_smem, {warpid, 0}));
        ...
        atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, q_head_idx, 2, 0}, warpid);
        auto attn_i_smem_subtile = subtile_inplace<WARP_SIZE_KV, DOT_SLICE_QO>(attn_i_smem, {warpid, 0});
        store(attn_i_smem_subtile, dP_ij_bf16_accum_row);
    ...
    for (int i = 1; i < num_steps - 1; ++i, tic ^= 1, toc ^= 1) {
        ...
        // dot slice 0
        {
            ...
            atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, q_head_idx, (i - 1) * 4 + 3, 0}, warpid);
            auto attn_i_smem_subtile = subtile_inplace<WARP_SIZE_KV, DOT_SLICE_QO>(attn_i_smem, {warpid, 0});
            store(attn_i_smem_subtile, dP_ij_bf16_accum_row);
            load(dO_i_col, subtile_inplace<DOT_SLICE_QO, D>(dO_i_smem[tic][0], {0, 0}));
            load(Q_i_col, subtile_inplace<DOT_SLICE_QO, D>(Q_i_smem[tic][0], {0, 0}));
    ...
    // dot slice 0
    {
        ...
        atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, q_head_idx, (num_steps - 2) * 4 + 3, 0}, warpid);
        auto attn_i_smem_subtile = subtile_inplace<WARP_SIZE_KV, DOT_SLICE_QO>(attn_i_smem, {warpid, 0});
        store(attn_i_smem_subtile, dP_ij_bf16_accum_row);
        load(dO_i_col, subtile_inplace<DOT_SLICE_QO, D>(dO_i_smem[tic][0], {0, 0}));
        load(Q_i_col, subtile_inplace<DOT_SLICE_QO, D>(Q_i_smem[tic][0], {0, 0}));
